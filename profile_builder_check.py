'''
Use on an ACP or KS fasta (either the ACPs from top_acp_*_hits.fa or the KSs from ks_which_follow_acp_hits.fa from ks_finder_v2 [or vice-versa]) 
unaligned fasta of ACPs or KSs used to build the hmm profile which was used to search UniProtKB to get the list of ACPs or KSs.
...
Usage: python acp_validator.py fasta seqs_which_built_profile
'''
...
if len(known_acps_dict) == len(acps_found):
	...
	print('\nThe following Seqs were not found in %s' % found_acps)
...
with open('.'.join(found_acps.split('.')[0:-1]) + '_known_seqs_marked.fa', 'w') as output_file:
